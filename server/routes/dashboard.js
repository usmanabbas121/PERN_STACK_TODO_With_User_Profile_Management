...
router.get("/", authorization, async (req, res) => {
  try {
    const user = await pool.query(
      "SELECT u.user_name,u.user_email,u.user_id, t.todo_id, t.description FROM  users AS u INNER JOIN todos AS t ON u.user_id = t.user_id WHERE u.user_id =$1",
      [req.user]
    );
...
router.put("/users/:id", authorization, async (req, res) => {
  try {
    ...
    if (updateUser.rows.length === 0) {
      return res.json("This user is not you");
    }
